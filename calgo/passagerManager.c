...
/**
 * @brief fichier passagerManager contenant les fonctions li�es au passager
 * @author Killian
 * @author Axel
 *
 */
...
void tri_passager(Passager tableau_passagers[])
{
    ...
    // Tri d�croissant des billets dans le tableau
...
int calculer_age(char date_naissance[])
{
    ...
    // Calculer l'�ge du passager
    age = date.tm_year + 1900 - annee; // Soustraire l'ann�e de naissance � l'ann�e actuelle
    if (date.tm_mon + 1 < mois || // Si mois inf�rieur
        (date.tm_mon + 1 == mois && date.tm_mday < jour)) // Si jour inf�rieur
    {
        age--; // D�cr�menter l'�ge de 1
    }

    // Retourner l'�ge du passager
    return age;
}
